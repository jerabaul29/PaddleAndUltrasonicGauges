...
class grabb_serial_values(object):
    ...
    def save_cleaned_dict_numpy_csv(self,path):
        ...
        min_size = min(self.dict_numpy_cleaned['A'].shape[0],self.dict_numpy_cleaned['B'].shape[0],self.dict_numpy_cleaned['C'].shape[0],self.dict_numpy_cleaned['D'].shape[0])
        matrix_save = np.zeros((min_size, 4))
